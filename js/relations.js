// relations.js - Gestion des relations Instagram (Unfollowers, Fans, En attente)
const relations = {
    data: {
        following: [],
        followers: [],
        pending: [], // NOUVEAU : demandes en attente
        unfollowers: [],
        fans: [], // NOUVEAU : personnes qui me suivent mais que je ne suis pas
        marked: new Set(),
        ...
        currentTab: 'unfollowers' // NOUVEAU : tab actuel
    },
    ...
    updateCounts() {
        ...
        document.getElementById('unfollowersTabCount').textContent = this.data.unfollowers.length;
        document.getElementById('fansTabCount').textContent = this.data.fans.length;
        document.getElementById('pendingTabCount').textContent = this.data.pending.length;
    },

    // NOUVEAU : Changer de tab
    switchTab(tabName) {
        console.log('🔄 Changement de tab:', tabName);
        ...
        document.querySelectorAll('.relations-content').forEach(content => {
            content.classList.remove('active');
        });
        document.getElementById(`${tabName}Content`).classList.add('active');
        
        // Afficher les données du tab approprié
        this.displayCurrentTab();
    },

    // NOUVEAU : Afficher le contenu du tab actuel
    displayCurrentTab() {
        switch(this.data.currentTab) {
            case 'unfollowers':
                this.displayUnfollowers();
                break;
            case 'fans':
                this.displayFans();
                break;
    ...
    // NOUVEAU : Afficher les fans
    displayFans() {
        if (this.data.fans.length === 0) {
            document.getElementById('fansResults').style.display = 'none';
            document.getElementById('emptyFans').style.display = 'flex';
            return;
        }
        
        document.getElementById('fansResults').style.display = 'block';
        document.getElementById('emptyFans').style.display = 'none';
        
        document.getElementById('fansCount').textContent = this.data.fans.length;
        ...
        this.data.fans.forEach(fan => {
            ...
            item.innerHTML = `
                <a href="https://www.instagram.com/${fan}" target="_blank" class="unfollower-name">
                    ${fan}
                </a>
            `;
    ...
    // NOUVEAU : Afficher les demandes en attente
    displayPending() {
        if (this.data.pending.length === 0) {
            document.getElementById('pendingResults').style.display = 'none';
            document.getElementById('emptyPending').style.display = 'flex';
            return;
        }
        
        document.getElementById('pendingResults').style.display = 'block';
        document.getElementById('emptyPending').style.display = 'none';
        
        document.getElementById('pendingCount').textContent = this.data.pending.length;
        
        // Générer la liste
        const pendingList = document.getElementById('pendingList');
        pendingList.innerHTML = '';
        
        this.data.pending.forEach(request => {
            const item = document.createElement('div');
            item.className = 'unfollower-item';
            item.innerHTML = `
                <a href="https://www.instagram.com/${request.username}" target="_blank" class="unfollower-name">
                    ${request.username}
                </a>
                <span class="pending-date">${request.date || ''}</span>
            `;
            pendingList.appendChild(item);
        });
    },

    // Afficher les unfollowers (fonction existante renommée)
    displayUnfollowers() {
        if (this.data.unfollowers.length === 0) {
            document.getElementById('unfollowersResults').style.display = 'none';
            document.getElementById('emptyUnfollowers').style.display = 'flex';
            return;
        }
    ...
    async processZipFileForContacts(file) {
        ...
        try {
            ...
            contents.forEach((relativePath, zipEntry) => {
                ...
                } else if (relativePath.includes('pending_follow_requests.json')) { // NOUVEAU
                    pendingFile = zipEntry;
                }
            });
            ...
            // NOUVEAU : Parser le fichier pending si disponible
            let pendingData = null;
            if (pendingFile) {
                const pendingText = await pendingFile.async('text');
                pendingData = JSON.parse(pendingText);
                console.log('✅ Fichier pending_follow_requests.json trouvé');
            } else {
                console.log('ℹ️ Fichier pending_follow_requests.json non trouvé (optionnel)');
            }
            ...
            // NOUVEAU : Calculer les fans (personnes qui me suivent mais que je ne suis pas)
            this.data.fans = followersList.filter(f => !followingSet.has(f));
            console.log('⭐ Fans calculés:', this.data.fans.length);
            
            // NOUVEAU : Parser les demandes en attente si disponible
            if (pendingData && pendingData.relationships_follow_requests_sent) {
                this.data.pending = pendingData.relationships_follow_requests_sent.flatMap(item => 
                    (item.string_list_data || []).map(req => ({
                        username: req.value,
                        date: req.timestamp ? new Date(req.timestamp * 1000).toLocaleDateString('fr-FR') : ''
                    }))
                );
                console.log('⏳ Demandes en attente:', this.data.pending.length);
            } else {
                this.data.pending = [];
